...
def _status_text(engine) -> str:
    ...
    lines = [
        ...
        f"threshold_full_sweep_enabled: {_fmt_bool(engine._config.threshold_full_sweep_enabled)}",
        f"threshold_only_compaction_enabled: {_fmt_bool(engine._config.threshold_only_compaction_enabled)}",
        f"configured_post_compaction_target_ratio: {status.get('configured_post_compaction_target_ratio', engine._config.post_compaction_target_ratio)}",
        f"post_compaction_target_ratio: {status.get('post_compaction_target_ratio', engine._config.post_compaction_target_ratio)}",
        f"post_compaction_target_ratio_source: {status.get('post_compaction_target_ratio_source', 'manual_or_default')}",
        f"post_compaction_target_tokens: {status.get('post_compaction_target_tokens', 0)}",
        f"matched_model_policy_key: {status.get('matched_model_policy_key') or '(none)'}",
